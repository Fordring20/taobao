...
/**
 * url访问：登录时会重新拼接url,方法：insert_add_url_uid
 */
...
if ($act == 'adsource')
{
    include_once('includes/cls_json.php');
    $json = new JSON;
    $result   = array('error' => 0, 'content' => '', 'type' => 0, 'cat_id' => 0);

    $result['content'] = get_wap_group('app启动轮播图1-1', 5);
...
{
        $arr = array( );
        $sql = "select ap.ad_width,ap.ad_height,ad.ad_id,ad.ad_name,ad.ad_code,ad.ad_link,ad.ad_id,ad.link_man,ad.end_time,ad.start_time,ad.link_email,ad.enabled from ".$GLOBALS['ecs']->table( "ecsmart_ad_position" )." as ap left join ".$GLOBALS['ecs']->table( "ecsmart_ad" )." as ad on ad.position_id = ap.position_id where ap.position_name='".$position.( "' and UNIX_TIMESTAMP()>ad.start_time and UNIX_TIMESTAMP()<ad.end_time and ad.enabled=1 limit ".$num );
        $res = $GLOBALS['db']->getAll( $sql );
        $http_url = "http://" . $_SERVER['HTTP_HOST'] . "/mobile/";
        $arr = array();
        foreach ($res as $key => $value)
        {
            if ($value['enabled'] == 1)
            {
                $value['image_url'] = $http_url . "data/afficheimg/".$value['ad_code'];
                $arr[] = $value;
            }
        }
        return $arr;
