 ...
 * @returns {Array}
 */
function addToCartth(goodsId, parentId, isnowbuy)

{
  ...
  if (formBuy){
    ...
    goods.number   = number;
    goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
    Ajax.call('flowth.php?step=add_to_cartth', 'goods=' + $.toJSON(goods), addToCartResponseth, 'POST', 'JSON');
}
...
function addToCartResponseth(result)
{
  if (result.error > 0)
  {
    // 如果需要缺货登记，跳转
	...
	var cartInfoth= document.getElementById('ECS_CARTINFOTH');
        var cart_url = 'flowth.php?step=cart';
        if (result.one_step_buy == '1') {
	   location.href = cart_url;
       }else{
    if (cartInfoth){
      cartInfoth.innerHTML = result.goods_number;
