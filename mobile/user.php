...
/* 显示页面的action列表 */
$ui_arr = array(
	'login','profile','order_list','order_detail','tg_order','tg_login','address_list','collection_list','message_list','tag_list',
...
function action_login()
{
	...
	$gotype = '0';
	if (isset($_GET['gotype']))
	{
		$gotype = trim($_GET['gotype']);
	}
	$smarty->assign('gotype', $gotype);
...
function action_act_login()
{
	...
	/* 代码增加2014-12-23 by bbs.ahxcjy.com _star */
...
function action_logout()
{
	...
	if ($bktype == 'mdpwd')
	{
		show_message('密码修改成功，请重新登录！', array(
			'返回登录'
		), array(
			'user.php?gotype=mdpwd'
		), 'info');
	}
	else
	{
...
function action_order_list()
{
	...
	$sql = "select * from " . $ecs->table('order_info') . " where user_id = '" . $user_id . "' and shipping_status = 1";
	$dthorders = $db->GetAll($sql);
	foreach ($dthorders as $key => $val)
	{
		//待提货状态的订单处理
		$r = check_order_shouhuo($val['order_id']);
	}
...
function action_ajax_order_list(){
    ...
    {	
	    /*获取相应的虚拟物品是否存在*/
	    $card = $GLOBALS['db']->getRow("select card_sn from ".$GLOBALS['ecs']->table('virtual_goods_card')." where order_sn='".$val['order_sn']."'");	
	    if ($card != null)
	    {
			$smarty->assign('card_sn', $card['card_sn']);
	    }
	    else
	    {
			$smarty->assign('card_sn', 0);
	    }
...
function action_shouhou_order(){
    ...
    $sql = "select og.*,g.goods_thumb from " . $ecs->table('order_goods') . " as og, " . $ecs->table('goods') . " as g where og.goods_id=g.goods_id and order_id = '" . $order_id . "' limit 1";
    $goods = $db->getRow($sql);
    $goods['goods_thumb'] = '../' . $goods['goods_thumb'];
    $smarty -> assign('goods',$goods);
    $smarty -> assign('page_title', '申请售后_爱心购');
    ...
    $smarty -> display('shouhou_order.dwt');        
}

/* 申请售后提交执行 */
function action_shouhou_save()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$sql = "select g.*, o.order_sn from " . $ecs->table('order_goods') . " as g, " . $ecs->table('order_info') . " as o where g.order_id = o.order_id and rec_id = '" . $rec_id . "'";
	$goods = $db->getRow($sql);
	if (!$goods)
	{
		echo "<script>alert('此商品订单不存在！');location='user.php?act=my_shouhou';</script>";
        exit();
	}
	...
	$shwenti = intval($_POST['shwenti']);
	$message = $_POST['message'];
	$add_time = gmtime();
	$status = 0;
    $res_shouhou_id = $db->getOne("select id from ".$ecs->table('shouhou') ." where rec_id = $rec_id and user_id = $user_id");
    if($res_shouhou_id){
        echo "<script>alert('此商品您已经申请过售后！');location='user.php?act=my_shouhou';</script>";
        exit();
    }else{
	    $sql = "INSERT INTO " . $ecs->table('shouhou') . "(user_id,rec_id,order_id,order_sn, goods_id,goods_sn,goods_name, wentitype, message, add_time, status)" . "VALUES ('$user_id','$rec_id','$order_id','$order_sn', '$goods_id', '$goods_sn','$goods_name', '$shwenti', '$message', '$add_time', '$status')";
	    $db->query($sql);
	    $shouhou_id = $db->insert_id();
		// 处理图片
	    for($i=0;$i<$simgnum;$i++){
	            ...
	            { 
	                if(move_uploaded_file($v["tmp_name"],$path . $v["name"])){
	                    ...
	                    $img_url .= $path_img . ';';
	                        ...
	                        echo "<script>alert('图片上传失败，请重新提交！');self.location='user.php?act=shouhou_order&order_id=$order_id';</script>";
	                        exit();
	                }
	    ...
	    if (!empty($img_url))
	    {
	    	$sql = "update " . $ecs->table('shouhou') . " set img_url = '" . $img_url . "' where id = '" . $shouhou_id . "'";
	    	$db->query($sql);
	    }
	}

    $msg = '您的信息提交成功，等待客服处理！';
	echo "<script>alert('$msg');self.location='user.php?act=my_shouhou';</script>";
	exit();
}

/* 我的售后 */
function action_my_shouhou()
{
    //全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	...
	$smarty->assign('page_title', '我的售后_爱心购');
	$smarty->assign('shouhouflag', '1');

	$smarty->display('user_transhouhou.dwt');
}

function action_ajax_shouhou_list(){
	//全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
    ...
    $sql = "select s.*,g.*,o.pay_time,eg.goods_thumb,eg.suppliers_id from " . $ecs->table('shouhou') . " as s, " . $ecs->table('order_goods') . " as g, " . $ecs->table('order_info') . " as o, " . $ecs->table('goods') . " as eg where s.rec_id=g.rec_id and s.order_id=o.order_id and s.goods_id=eg.goods_id and s.user_id = '" . $user_id . "' $limit";
    $orders = $db->getAll($sql);
	
    foreach($orders as $key=>$val)
    {
    	$sql = "select suppliers_name from " . $ecs->table('suppliers') . " where suppliers_id = '" . $val['suppliers_id'] . "'";
    	$val['suppliers_name'] = $db->getOne($sql);
    	$val['formated_goods_price'] = number_format($val['goods_price'] * $val['goods_number'], 2);
    	$val['pay_time'] = local_date("Y-m-d", $val['pay_time']);
    	$val['goods_thumb'] = '../' . $val['goods_thumb'];
        $GLOBALS['smarty']->assign('order',$val);
        $result[]['info']  = $GLOBALS['smarty']->fetch('library/user_shouhou_list.lbi');
    }
    die($json->encode($result));
...
function action_order_detail()
{
	...
	/*获取相应的虚拟物品是否存在*/
	$card = $GLOBALS['db']->getRow("select card_sn from ".$GLOBALS['ecs']->table('virtual_goods_card')." where order_sn='".$order['order_sn']."'");	
	if($card != null){
...
//	}
    ...
    $ppoint = $order['pickpoint'];
    $sql = "select * from " . $GLOBALS['ecs']->table('pickup_point') . " where id='" . $ppoint . "'";
    $thpoint = $GLOBALS['db']->getRow($sql);
    $sql = "select region_name from " . $GLOBALS['ecs']->table('region') . " where region_id='" . $thpoint['province_id'] . "'";
    $thpoint['sheng'] = $GLOBALS['db']->getOne($sql);
    $sql = "select region_name from " . $GLOBALS['ecs']->table('region') . " where region_id='" . $thpoint['city_id'] . "'";
    $thpoint['shi'] = $GLOBALS['db']->getOne($sql);
    $sql = "select region_name from " . $GLOBALS['ecs']->table('region') . " where region_id='" . $thpoint['district_id'] . "'";
    $thpoint['qu'] = $GLOBALS['db']->getOne($sql);
    $smarty->assign('thpoint', $thpoint);
...
function action_download(){
	...
	$smarty->assign('page_title', '下载APP');

	$smarty->display('user_download.dwt');
} 

/* 修改密码界面 */
function action_modifypwd()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$smarty->assign('page_title', '修改密码');

	$smarty->display('user_mdpassword.dwt');
}

/* 修改密码 */
function action_mdpwdupdate()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	{
		exit(json_encode(array('res' => 0, 'msg' => '请输入密码！')));
	}
	...
	if (!$u)
	{
		exit(json_encode(array('res' => 0, 'msg' => '登录超时，请重新登录！')));
	}

	if (!empty($u['ec_salt']))
	{
		$oldpwd = md5($oldpwd . $u['ec_salt']);
	}
	...
	{
		exit(json_encode(array('res' => 0, 'msg' => '新密码长度不少于6位！')));
	}

	$sql = "update " . $ecs->table('users') . " set password = '" . $newpwd . "', ec_salt = 0 where user_id = '" . $user_id . "'";
	$res = $db->query($sql);
	if (!$res)
	{
		exit(json_encode(array('res' => 0, 'msg' => '网络延迟，请重试！')));
	}
...
function action_act_account()
{
	...
	/* 变量初始化 */
	$surplus = array(
		'user_id' => $user_id,'rec_id' => ! empty($_POST['rec_id']) ? intval($_POST['rec_id']) : 0,'process_type' => isset($_POST['surplus_type']) ? intval($_POST['surplus_type']) : 0,'payment_id' => isset($_POST['payment_id']) ? intval($_POST['payment_id']) : 0,'user_note' => isset($_POST['user_note']) ? trim($_POST['user_note']) : '','amount_txr_xm' => isset($_POST['amount_txr_xm']) ? trim($_POST['amount_txr_xm']) : '','amount_txr_yhkmc' => isset($_POST['amount_txr_yhkmc']) ? trim($_POST['amount_txr_yhkmc']) : '','amount_txr_yhkkh' => isset($_POST['amount_txr_yhkkh']) ? trim($_POST['amount_txr_yhkkh']) : '','amount_txr_zfb' => isset($_POST['amount_txr_zfb']) ? trim($_POST['amount_txr_zfb']) : '','amount' => $amount
	);
...
function action_vc_login_act()
{
	...
	$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('user_account') . ' (user_id, admin_user, amount, add_time, paid_time, admin_note,amount_txr_xm,amount_txr_yhkmc,amount_txr_yhkkh,amount_txr_zfb, user_note, process_type, payment, is_paid)' . " VALUES ('$user_id', '', '$vcrow[type_money]', '" . gmtime() . "', '" . gmtime() . "','$amount_txr_xm','$amount_txr_yhkmc','$amount_txr_yhkkh','$amount_txr_zfb','', '', '储值卡充值', '0', '储值卡号：$vc_sn', 1)";
	$GLOBALS['db']->query($sql);
...
function check_order_shouhuo($order_id)
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$sql = "select count(*) as num from " . $GLOBALS['ecs']->table('order_goods') . " where order_id = '" . $order_id . "' and ship_status != 2";
	$count = $GLOBALS['db']->GetOne($sql);
	if ($count == 0)
	{
		autocheck_affirm_received($order_id);
		return true;	//需要修改确认收货
	}
	else
	{
		return false;	//不需要修改确认收货
	}
}

/* 检查确认收货 */
function autocheck_affirm_received($order_id)
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
