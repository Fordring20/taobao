...
function action_check_username ()
{
	...
	if(empty($username))
	{
		show_message('请输入登录的手机号！', '返回', 'findPwd.php?act=index', 'info');
	}
	...
	if(is_mobile_phone($username))
	{
		...
		if($index > 1)
		{
			show_message('本网站有多个会员ID绑定了和您相同的手机号，请联系客服为您解决。', $_LANG['relogin_lnk'], 'findPwd.php', 'error');
		}
		else if($index == 1)
		{
			if($user_id)
	...
	if(! $username_exist)
	{
		show_message('您输入的手机号不正确，请核对后重新输入。', $_LANG['relogin_lnk'], 'findPwd.php', 'error');
	}
	
	// 获取用户信息，判断用户是否验证了手机、邮箱
	// $sql = "select user_id, user_name, email, mobile_phone from " . $ecs->table('users') . " where user_id = '" . $user_id . "'";
	$sql = "select user_id, user_name, mobile_phone from " . $ecs->table('users') . " where user_id = '" . $user_id . "'";
	
	$row = $db->getRow($sql);
	
	if($row == false)
	{
		show_message('您输入的手机号不存在，请核对后重新输入。', $_LANG['relogin_lnk'], 'findPwd.php', 'error');
	}
...
function action_validate ()
{
	...
	if(!isset($_SESSION['find_password']))
	{
		...
		exit(json_encode(array('error' => 1, 'content' => '手机号不能为空', 'url' => 'findPwd.php')));
	}
...
/* 代码增加2014-12-23 by bbs.ahxcjy.com _end */
