...
//start by z1988.com
if ( $_REQUEST['step'] == 'cart' && $_SESSION['user_id'] > 0){
	//security.php?act=mobile_binding
	...
	/* 是否开启强制手机绑定 */
	if ($_CFG['z1988_mobile_cons'] == '1' && (!$row || !is_mobile($row['mobile_phone'])))
	{
...
{	
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
        ...
        if(isset($total['real_goods_count']) && $total['real_goods_count'] == 0){
            	$order['extension_code']='virtual_good';
        }else{    
	...
	//$GLOBALS['db'] -> query("unlock tables");
		...
		/* 客户下订单时给商家发送短信提醒 */
		//修改为商家手机号码
		if ($_CFG['z1988_order_placed'] == '1' && $_CFG['z1988_shop_mobile'] != '')
		{
			require_once(ROOT_PATH . 'includes/lib_sms.php');
		
			$sms_content	= $GLOBALS['_CFG']['z1988_sms_order_placed_value'];
			$sms_tempid		= isset( $GLOBALS['_CFG']['z1988_order_placed_tempid']) ? $GLOBALS['_CFG']['z1988_order_placed_tempid'] : '';
			$sms_data		= array($order['order_sn'], $order['consignee'], $order['tel'].' '.$order['mobile']);
			$sms_content	= str_replace(array('{$order_sn}','{$consignee}','{$tel}'), $sms_data , $sms_content);
			
			sendsms($_CFG['z1988_shop_mobile'], $sms_content, $sms_data, $sms_tempid );

		}
		if ($_CFG['z1988_customer_placed'] == '1'){ 
			/* 获取用户手机号 */
			$sql = "SELECT user_id, mobile_phone FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id='$order[user_id]' LIMIT 1";
				$row_user = $GLOBALS['db']->getRow($sql);

			/* 客户下订单时给客户发送短信提醒 */
			if ( $row_user['mobile_phone'] != '' )
			{
				require_once(ROOT_PATH . 'includes/lib_sms.php');

				$sms_content	= $GLOBALS['_CFG']['z1988_customer_placed_value'];
				$sms_tempid		= isset( $GLOBALS['_CFG']['z1988_customer_placed_tempid']) ? $GLOBALS['_CFG']['z1988_customer_placed_tempid'] : '';
				$sms_data		= array($order['order_sn'], $order['consignee'], $order['tel'].' '.$order['mobile']);
				$sms_content	= str_replace(array('{$order_sn}','{$consignee}','{$tel}'), $sms_data , $sms_content);
				
				sendsms($row_user['mobile_phone'], $sms_content, $sms_data, $sms_tempid );
		...
		if ($send)
		{
			/* 客户付款时给商家发送短信提醒 */
			//修改修改为商家手机号码
			if ($_CFG['z1988_order_payed'] == '1' && $_CFG['z1988_shop_mobile'] != '' )
			{
				require_once(ROOT_PATH . 'includes/lib_sms.php');

				$sms_content	= $GLOBALS['_CFG']['z1988_order_payed_value'];
				$sms_tempid		= isset( $GLOBALS['_CFG']['z1988_order_payed_tempid']) ? $GLOBALS['_CFG']['z1988_order_payed_tempid'] : '';
				$sms_data		= array($order['order_sn'], $order['consignee'], $order['tel'].' '.$order['mobile']);
				$sms_content	= str_replace(array('{$order_sn}','{$consignee}','{$tel}'), $sms_data , $sms_content);
			...
			if ($_CFG['z1988_customer_payed'] == '1'){ 
				...
				if ( $row_user['mobile_phone'] != '' )
				{
					...
					$sms_content	= $GLOBALS['_CFG']['z1988_customer_payed_value'];
					$sms_tempid		= isset( $GLOBALS['_CFG']['z1988_customer_payed_tempid']) ? $GLOBALS['_CFG']['z1988_customer_payed_tempid'] : '';

					$sms_data		= array($order['order_sn'], $order['consignee'], $order['tel'].' '.$order['mobile'], date('Y-m-d H:i'));
					$sms_content	= str_replace(array('{$order_sn}','{$consignee}','{$tel}','{$time}'), $sms_data , $sms_content);
...
//                    }
	        ...
	        if ($virtual_goods AND $flow_type != CART_GROUP_BUY_GOODS)
	        {
	            ...
	            {         
						...
						$sms_tempid		= '';
						$sms_data		= array($supplier_name, $val['goods_name'], $card_sn, local_date('Y-m-d',$val['valid_date']) );
						$sms_content	= sprintf($_LANG['mobile_virtual_template'], $val['goods_name'], $card_sn,local_date('Y-m-d',$val['valid_date']));
						
						$result = sendsms($_REQUEST['mobile_phone'], $sms_content, $sms_data, $sms_tempid );
						// by z1988.com
