 ...
 * ECSHOP 购物流程
 * ============================================================================
 * * 版权所有 2008-2015 折翼天使资源社区网络服务中心，并保留所有权利。
 * 网站地址: http://bbs.ahxcjy.com;
 * ----------------------------------------------------------------------------
...
elseif ($_REQUEST['act']=='delAddress')
{
	...
	$consignee_list = get_consignee_list_ahxcjy();
...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$address_ahxcjy = $json->decode($_POST['address']);
	$consignee = array(
            'address_id'    => empty($address_ahxcjy->address_id) ?  '0'  :   intval($address_ahxcjy->address_id),
            'consignee'     => empty($address_ahxcjy->consignee)  ? '' :   compile_str(trim($address_ahxcjy->consignee)),
            'country'       => empty($address_ahxcjy->country)    ? '' :   intval($address_ahxcjy->country),
            'province'      => empty($address_ahxcjy->province)   ? '' :   intval($address_ahxcjy->province),
            'city'          => empty($address_ahxcjy->city)       ? '' :   intval($address_ahxcjy->city),
            'district'      => empty($address_ahxcjy->district)   ? '' :   intval($address_ahxcjy->district),
            'email'         => empty($address_ahxcjy->email)      ? '' :   compile_str($address_ahxcjy->email),
            'address'       => empty($address_ahxcjy->address)    ? '' :   compile_str($address_ahxcjy->address),
            'zipcode'       => empty($address_ahxcjy->zipcode)    ? '' :   compile_str(make_semiangle(trim($address_ahxcjy->zipcode))),
            'tel'           => empty($address_ahxcjy->tel)        ? '' :   compile_str(make_semiangle(trim($address_ahxcjy->tel))),
            'mobile'        => empty($address_ahxcjy->mobile)     ? '' :   compile_str(make_semiangle(trim($address_ahxcjy->mobile))),
     );
	...
	$result = array('error' => 0, 'message' => '', 'content' => '', 'closediv'=>$address_ahxcjy->closediv);    
	$consignee_list = get_consignee_list_ahxcjy();
	$smarty->assign('consignee_list', $consignee_list);
	$result['content'] =  $smarty->fetch("library/address_list.lbi");
	...
	if ($address_ahxcjy->shipping_bian=='0' && $address_ahxcjy->address_id>0 && $address_ahxcjy->address_id==$_SESSION['flow_consignee']['address_id'])
	{
		$sql = "SELECT * ".
                    " FROM " . $GLOBALS['ecs']->table('user_address') . 
                    " WHERE  address_id = '".$address_ahxcjy->address_id."' ";
		$consignee = $GLOBALS['db']->getRow($sql);
		$_SESSION['flow_consignee'] = $consignee;
	}
	if ($address_ahxcjy->shipping_bian=='1' || ($address_ahxcjy->shipping_bian=='0' && $address_ahxcjy->address_id>0 && $address_ahxcjy->address_id==$_SESSION['flow_consignee']['address_id']))
	{
	$order = flow_order_info();
	$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
...
function get_consignee_list_ahxcjy()
{
	$consignee = get_consignee($_SESSION['user_id']);
	$sql="SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '". $_SESSION['user_id'] ."' order by address_id ";
	$consignee_list_ahxcjy = $GLOBALS['db']->getAll($sql);
	foreach ($consignee_list_ahxcjy  as $cons_key => $cons_val)
	{
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] = $cons_val['consignee']."<br>";
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  get_region_info($cons_val['province'])."-";
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  get_region_info($cons_val['city'])."-";
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  get_region_info($cons_val['district'])."&nbsp;";
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
				$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  "<br>".($cons_val['tel'] != '--' ? $cons_val['tel'] : $cons_val['mobile']);
				if ($consignee['address_id'] == $cons_val['address_id'])
				{
					$consignee_list_ahxcjy[$cons_key]['def_addr'] =1;
					$have_def_addr=1;
				}
	}
	if ( count($consignee_list_ahxcjy) && !$have_def_addr){ $consignee_list_ahxcjy[0]['def_addr'] =1; }
	return 	$consignee_list_ahxcjy;	
}
/* 代码增加_end  By bbs.ahxcjy.com  */
...
//-- 添加自提点到订单
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'zitidian')
{
    $province = empty($_GET['province']) ? -1 : intval($_GET['province']);
    $city = empty($_GET['city']) ? -1 : intval($_GET['city']);
    $district = empty($_GET['district']) ? -1 : intval($_GET['district']);
    $smarty->assign('selsheng', $province);
    $smarty->assign('selshi', $city);
    $smarty->assign('selqu', $district);
    ...
    $sheng = $GLOBALS['db']->getAll($sql);
    $smarty->assign('sheng', $sheng);
    $sql = "select * from " . $GLOBALS['ecs']->table('region') . " where parent_id = '" . $province . "' and region_type = 2";
    $shi = $GLOBALS['db']->getAll($sql);
    $smarty->assign('shi', $shi);
    $sql = "select * from " . $GLOBALS['ecs']->table('region') . " where parent_id = '" . $city . "' and region_type = 3";
    $qu = $GLOBALS['db']->getAll($sql);
    $smarty->assign('qu', $qu);

    $sql = "select * from " . $GLOBALS['ecs']->table('pickup_point') . $where;
    $ppoint = $GLOBALS['db']->getAll($sql);
    foreach ($ppoint as $key => $value)
    {
        $ppoint[$key]['img_url'] = empty($value['img_url']) ? './themesmobile/ahxcgg_mobile/images/ztd_logo.png' : '../data/ztdimg/' . $value['img_url'];
        $ppoint[$key]['thshijian'] = str_replace(' ', '', $value['thshijian']);
    }

    $pcount = count($ppoint);
    $smarty->assign('ppoint', $ppoint);
    $smarty->assign('pcount', $pcount);

    $smarty->display('flow_zitidian.dwt');
    die();
}
/*------------------------------------------------------ */
//-- 查看自提点列表
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'zitidianlist')
{
    $province = empty($_GET['province']) ? -1 : intval($_GET['province']);
    $city = empty($_GET['city']) ? -1 : intval($_GET['city']);
    $district = empty($_GET['district']) ? -1 : intval($_GET['district']);
    ...
    $smarty->assign('ppoint', $ppoint);
    $smarty->assign('pcount', $pcount);
    $smarty->assign('page_title', '自提点列表_爱心购');

    $smarty->display('flow_zitidianlist.dwt');
    die();
}
/*------------------------------------------------------ */
//-- 自提点申请
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'zitidiansq')
{
    $province = empty($_GET['province']) ? -1 : intval($_GET['province']);
    $city = empty($_GET['city']) ? -1 : intval($_GET['city']);
    $district = empty($_GET['district']) ? -1 : intval($_GET['district']);
    ...
    $ztdname = empty($_GET['ztd']) ? '' : trim($_GET['ztd']);
    $lxrname = empty($_GET['lxr']) ? '' : trim($_GET['lxr']);
    $lxrphone = empty($_GET['lxrp']) ? '' : trim($_GET['lxrp']);
    $smarty->assign('ztdname', $ztdname);
    $smarty->assign('lxrname', $lxrname);
    $smarty->assign('lxrphone', $lxrphone);

    $addaddress = empty($_GET['address']) ? '' : trim($_GET['address']);
    $addlng = empty($_GET['lng']) ? -1 : trim($_GET['lng']);
    $addlat = empty($_GET['lat']) ? -1 : trim($_GET['lat']);
    $smarty->assign('addaddress', $addaddress);
    $smarty->assign('addlng', $addlng);
    $smarty->assign('addlat', $addlat);
    ...
    $qu = $GLOBALS['db']->getAll($sql);
    $smarty->assign('qu', $qu);
    $smarty->assign('page_title', '网点申报_爱心购');

    $smarty->display('flow_zitidianshenqing.dwt');
    die();
}
/*------------------------------------------------------ */
//-- 自提点申请增加插入
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'zitidianadd')
{
    include_once('includes/cls_json.php');
    $result = array('error' => 0, 'message' => '', 'content' => '', 'goods_id' => '');
    $json  = new JSON;

    if (empty($_POST['ztdname']) || empty($_POST['lxrname']) || empty($_POST['lxrphone']) || empty($_POST['address']))
    {
        $result['error'] = 99;
        $result['message'] = '请输入完整的信息！';
        die($json->encode($result));
    }
    
    $ztdname = trim($_POST['ztdname']);
    $lxrname = trim($_POST['lxrname']);
    $lxrphone = trim($_POST['lxrphone']);
    $sheng = empty($_POST['sheng']) ? '0' : trim($_POST['sheng']);
    $shi = empty($_POST['shi']) ? '0' : trim($_POST['shi']);
    $qu = empty($_POST['qu']) ? '0' : trim($_POST['qu']);
    $address = trim($_POST['address']);
    $lng = empty($_POST['lng']) ? 0 : $_POST['lng'];
    $lat = empty($_POST['lat']) ? 0 : $_POST['lat'];

    $sql = "select region_id from " . $GLOBALS['ecs']->table('region') . " where INSTR('" . $sheng . "', region_name) > 0";
    $shengid = $GLOBALS['db']->getOne($sql);
    $sql = "select region_id from " . $GLOBALS['ecs']->table('region') . " where INSTR('" . $shi . "', region_name) > 0 and parent_id = '" . $shengid . "'";
    $shiid = $GLOBALS['db']->getOne($sql);
    $sql = "select region_id from " . $GLOBALS['ecs']->table('region') . " where INSTR('" . $qu . "', region_name) > 0 and parent_id = '" . $shiid . "'";
    $quid = $GLOBALS['db']->getOne($sql);

    $save['shop_name'] = $ztdname;
    $save['address'] = $address;
    $save['lxrphone'] = $lxrphone;
    $save['sqlxr'] = $lxrname;
    $save['province_id'] = $shengid;
    $save['city_id'] = $shiid;
    $save['district_id'] = $quid;
    $save['tuanzhang_id'] = $tz['id'];
    $save['lng'] = $lng;
    $save['lat'] = $lat;
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('pickup_point_sq'), $save, 'INSERT');
...
//-- 自提点申请地址选择
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'zitidiandzsel')
{
    $ztdname = empty($_GET['ztd']) ? '' : trim($_GET['ztd']);
    $lxrname = empty($_GET['lxr']) ? '' : trim($_GET['lxr']);
    $lxrphone = empty($_GET['lxrp']) ? '' : trim($_GET['lxrp']);
    ...
    $smarty->assign('page_title', '网点申报_爱心购');
    $smarty->display('flow_zitidianshenqingdz.dwt');
    die();
}
/*------------------------------------------------------ */
//-- 自提点查看地图
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'zitidianditu')
{
    $id = empty($_GET['id']) ? 0 : trim($_GET['id']);

    $sql = "select * from " . $GLOBALS['ecs']->table('pickup_point') . " where id='" . $id . "'";
    $ppoint = $GLOBALS['db']->getRow($sql);

    $sql = "select region_name from " . $GLOBALS['ecs']->table('region') . " where region_id='" . $ppoint['province_id'] . "'";
    $sheng = $GLOBALS['db']->getOne($sql);
    $sql = "select region_name from " . $GLOBALS['ecs']->table('region') . " where region_id='" . $ppoint['city_id'] . "'";
    $shi = $GLOBALS['db']->getOne($sql);
    $sql = "select region_name from " . $GLOBALS['ecs']->table('region') . " where region_id='" . $ppoint['district_id'] . "'";
    $qu = $GLOBALS['db']->getOne($sql);

    $smarty->assign('sheng', $sheng);
    $smarty->assign('shi', $shi);
    $smarty->assign('qu', $qu);
    $smarty->assign('address', $ppoint['address']);
    $smarty->assign('ppoint', $ppoint);

    $smarty->assign('page_title', '网点地图_爱心购');
    $smarty->display('flow_zitidianditu.dwt');
    die();
}
/*------------------------------------------------------ */
//-- 心耀龙城
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'axqiuzhu')
{
    $id = empty($_GET['id']) ? 0 : intval($_GET['id']);
    $smarty->assign('cat_id', $id);
    $smarty->assign('axqzflag', 1);

    setcookie('axqz_page', 1);

    $smarty->assign('wap_group_ad', get_wap_group('手机端求助广告1-1',1));  //手机端求助广告1-1

    $shankuan = get_order_shankuan();
    $axmoney = number_format($shankuan['amount'], 2, '.', ',');
    $smarty->assign('axmoney', $axmoney);

    $sql = "select * from " . $GLOBALS['ecs']->table('article') . " where cat_id='" . $id . "'";
    $goods_list = $GLOBALS['db']->getAll($sql);
    $smarty->assign('goods_list', $goods_list);

    $smarty->assign('page_title', '心耀龙城_爱心购');
    $smarty->display('category_axqz.dwt');
    die();
}
/*------------------------------------------------------ */
//-- 心耀龙城事迹刷新
/*------------------------------------------------------ */
if ($_REQUEST['step'] == 'ajaxqiuzhu')
{
    include('includes/cls_json.php');

    $size = 10;
    $page = empty($_COOKIE['axqz_page']) ? 1 : $_COOKIE['axqz_page'];
    $start = ($page - 1) * $size;
    $limit = "limit $start, $size";

    setcookie('axqz_page', $page + 1);
    ...
    $sql = "select * from " . $GLOBALS['ecs']->table('article') . " where cat_id='" . $cat_id . "' order by article_id desc $limit";
    $articlelist = $GLOBALS['db']->getAll($sql);

    foreach ($articlelist as $val) {
        $smarty->assign('article', $val);
        $iszhichi = 0;
        if ($_SESSION['user_id'] > 0)
        {
            $sql = "select count(*) from " . $GLOBALS['ecs']->table('article_hot') . " where user_id = '" . $_SESSION['user_id'] . "' and article_id = '" . $val['article_id'] . "'";
        ...
        $smarty->assign('iszhichi', $iszhichi);
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('article_hot') . " where article_id = '" . $val['article_id'] . "'";
        $count = $GLOBALS['db']->getOne($sql);
        $smarty->assign('axcount', $count);
        $res[]['info'] = $GLOBALS['smarty']->fetch('library/goods_list_ajaxqz.lbi');
    }
    die($json->encode($res));
}
/*------------------------------------------------------ */
...
if ($_REQUEST['step'] == 'axqzxianax')
{
    ...
    $sql = "select * from " . $GLOBALS['ecs']->table('article_hot') . " where article_id = '" . $id . "' and user_id = '" . $_SESSION['user_id'] . "'";
    $articlehot = $GLOBALS['db']->getAll($sql);
    if ($articlehot)
    {
        $result['error'] = 99;
        $result['message'] = '您已为此事迹献过爱心了！';
        die($json->encode($result));
    }
    ...
    $save['article_id'] = $id;
    $save['time'] = gmtime();
    $r = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('article_hot'), $save, 'INSERT');
    if ($r)
    {
        $sql = "update " . $GLOBALS['ecs']->table('article') . " set hot = hot + 1 where article_id = '" . $id . "'";
...
if ($_REQUEST['step'] == 'axqzinfo')
{
    ...
    if ($_SESSION['user_id'] > 0)
    {
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('article_hot') . " where user_id = '" . $_SESSION['user_id'] . "' and article_id = '" . $id . "'";
        $count = $GLOBALS['db']->getOne($sql);
        if ($count > 0)
        {
            $iszhichi = 1;
    ...
    $smarty->assign('iszhichi', $iszhichi);
    $sql = "select count(*) from " . $GLOBALS['ecs']->table('article_hot') . " where article_id = '" . $id . "'";
    $count = $GLOBALS['db']->getOne($sql);
    $smarty->assign('axcount', $count);

    $smarty->assign('page_title', '爱心购');
    $smarty->display('category_axqzinfo.dwt');
    die();
}
...
if ($_REQUEST['step'] == 'add_to_cart')
{
   include_once('includes/cls_json.php');
    ...
    if(!isset($goods->extCode) || $goods->extCode != 'pre_sale')
    {
    	...
    	//bbs.ahxcjy.com start add 2015-311-23 限购
	$time_xg_now=gmtime();
	...
	//bbs.ahxcjy.com end add 2015-11-23 限购
...
//-- 添加提货到购物车
/*------------------------------------------------------ */
elseif ($_REQUEST['step'] == 'add_to_cartth')
{
   include_once('includes/cls_json.php');
    $_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
	...
	if ( $row_xg['is_buy'] == 1 && $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
	{
		...
		else
		{
			...
			$num_cart_old_1=$GLOBALS['db']->getOne("select sum(goods_number) from ". $GLOBALS['ecs']->table('cartth') ." where " . $sql_where . " and goods_id= " . $goods->goods_id );
			$num_cart_old_2=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and  o.order_id = og.order_id and add_time > ". $row_xg['buymax_start_date'] ." and add_time < ". $row_xg['buymax_end_date'] ."  and og.goods_id = " . $goods->goods_id );
			$num_cart_old = $num_cart_old_1 + $num_cart_old_2 ;
			$num_total = $num_cart_old +  intval($goods->number);
			if ( $num_total > intval($row_xg['buymax']) )
...
$consignee['region'] = $db->getOne($sql); 
    ...
    $pickpoint = empty($_GET['point']) ? (empty($_SESSION['pickpoint']) ? 0 : intval($_SESSION['pickpoint'])) : intval($_GET['point']);
    if ($pickpoint)
    {
        $_SESSION['pickpoint'] = $pickpoint;
    }
    $sqlpiont = "select * from " . $GLOBALS['ecs']->table('pickup_point') . " where id='" . $pickpoint . "'";
    $point = $GLOBALS['db']->getRow($sqlpiont);
    $smarty->assign('pickpoint', $point);
    ...
    {	
		...
		foreach ($consignee_list_ahxcjy  as $cons_key => $cons_val)
		{
			...
			$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
			$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
			$consignee_list_ahxcjy[$cons_key]['address_short_name'] .=  "<br>".$cons_val['tel'];
			if ($consignee['address_id'] == $cons_val['address_id'])
			{
				$consignee_list_ahxcjy[$cons_key]['def_addr'] =1;
				$have_def_addr=1;
   ...
   $smarty->assign('name_of_region',   array($_CFG['name_of_region_1'], $_CFG['name_of_region_2'], $_CFG['name_of_region_3'], $_CFG['name_of_region_4']));
   $smarty->assign('consignee_list', $consignee_list_ahxcjy);
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
...
/*余额额支付密码_添加_START_bbs.ahxcjy.com*/
elseif ($_REQUEST['step'] == 'check_surplus_open')
{
...
{	
    ...
    /*检查提货点信息*/
    if (empty($_SESSION['pickpoint']))
    {
        ecs_header("Location: flow.php?step=zitidian\n");
        exit;
    }
    $pickpoint = intval($_SESSION['pickpoint']);
    $sql = "select * from " . $GLOBALS['ecs']->table('pickup_point') . " where id='" . $pickpoint . "'";
    $ppoint = $GLOBALS['db']->getRow($sql);
    if (!$ppoint)
    {
        ecs_header("Location: flow.php?step=zitidian\n");
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
	        ...
	        /*增值税发票_删除_START_bbs.ahxcjy.com*/
        	//'inv_type'        => $_POST['inv_type'],
        	//'inv_payee'       => trim($_POST['inv_payee']),
        	//'inv_content'     => $_POST['inv_content'],
			/*增值税发票_删除_END_bbs.ahxcjy.com*/
		...
		$order['defaultbank'] = $_POST['www_ahxcgg_com_bank'] ? trim($_POST['www_ahxcgg_com_bank']) : "";
		...
		/*检查配送方式是否选择*/
            ...
            if( $_SESSION['extension_code'] != 'virtual_good'){
        	    if(!isset($_POST['pay_ship'][$ckey])){
        	    	...
        	    	$shipid = $db->getOne("select shipping_id from ".$ecs->table('shipping')." limit 1");
        	    	if($shipid){
    ...
    //生成订单
	//$payment_www_com['www_ahxcjy_com_alipay_bank'] = $_POST['www_ahxcgg_com_bank'] ? trim($_POST['www_ahxcgg_com_bank']) : "www_ahxcjy_com";
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    /* 插入订单商品 下面这个SQL有修改 by www.ahxcjy.com 注意末尾那个字段 */
	    /* 代码增加_start  By bbs.ahxcjy.com */
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {   
                    /* 代码增加_start  By bbs.ahxcjy.com _sunlizhi*/
...
//                    }
                    ...
                    /* 代码增加_end  By bbs.ahxcjy.com _sunlizhi*/
...
function flow_drop_cart_goods($id)
{
    ...
    if ($row)
    {
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 bbs.ahxcjy.com
        //如果是超值礼包
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 bbs.ahxcjy.com
...
function get_order_shankuan()
{
    ...
    $strids = empty($strids) ? "''" : $strids;
    $sql = "select sum(goods_number) from " . $GLOBALS['ecs']->table('order_goods') . " where order_id in (" . $strids . ")";
    $gcount = $GLOBALS['db']->getOne($sql);
    $shankuan['count'] = $gcount ? $gcount : 0;
    $shankuan['amount'] = number_format($gcount * 0.1, 2);
    return $shankuan;
}
...
function get_wap_group( $position, $num )
{
        ...
        else
        {
            ...
            $arr[$row['ad_id']]['ss_time']  = '已结束';
                ...
                $arr[$row['ad_id']]['url'] = empty($row['ad_link']) ? 'javascript:void(0);' : $row['ad_link'];
