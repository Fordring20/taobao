 ...
 * ECSHOP 商品详情
 * ============================================================================
 * * 版权所有 2005-2012 折翼天使资源社区网络科技有限公司，并保留所有权利。
...
/* 代码增加 by bbs.ahxcjy.com strat */
require(dirname(__FILE__) . '/includes/lib_comment.php');
/* 代码增加 by bbs.ahxcjy.com end */
if ((DEBUG_MODE & 2) != 2)
{
    $smarty->caching = true;
}
...
/* 代码增加_start By  www.ahxcjy.com */
$sql_attr_www_ahxcjy_com="SELECT a.attr_id, ga.goods_attr_id FROM ". $GLOBALS['ecs']->table('attribute') . " AS a left join ". $GLOBALS['ecs']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $goods_id. "' order by ga.goods_attr_id ";
$goods_attr=$GLOBALS['db']->getRow($sql_attr_www_ahxcjy_com);
if($goods_attr){
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_gallery_attr')
{
	...
	$gallery_list_www_ahxcjy_com=get_goods_gallery_attr_www_ahxcjy_com($goods_id, $goods_attr_id);
	$gallery_content=array();
	$gallery_content['thumblist'] ='<ul>';
	foreach($gallery_list_www_ahxcjy_com as $gkey=>$gval)
	{
...
/* 代码增加_end   By www.ahxcjy.com */
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
		...
		$results_www_ahxcjy_com = $GLOBALS['db']->getAll($sql);
		$return_arr = array();
		foreach ($results_www_ahxcjy_com as $value_ahxcjy)
		{
			$return_arr[$value_ahxcjy['goods_attr_id']] = $value_ahxcjy['attr_value'];
		}
		...
		foreach ($properties['spe'] AS $skey_ahxcjy=>$sval_ahxcjy)
		{
			$hahaha_zhyh = 0;
			$sskey_www_ahxcjy_com = '-1';
			foreach ($sval_ahxcjy['values'] AS $sskey_ahxcjy=>$ssval_ahxcjy)
			{				
				if ( is_exist_prod($selected_first, $ssval_ahxcjy['id'], $prod_exist_arr))
				{ 
					$hahaha_zhyh = $hahaha_zhyh ? $hahaha_zhyh : $ssval_ahxcjy['id'];
					$sskey_www_ahxcjy_com = ($sskey_www_ahxcjy_com != '-1') ? $sskey_www_ahxcjy_com : $sskey_ahxcjy;
				}
				else
				{
					$properties['spe'][$skey_ahxcjy]['values'][$sskey_ahxcjy]['disabled'] = "disabled";
				}

				if ($skey_ahxcjy==$attr_id_gallery)
				{
					$goods_attr_id_qq = $ssval_ahxcjy['id'] ;
					...
					$properties['spe'][$skey_ahxcjy]['values'][$sskey_ahxcjy]['goods_attr_thumb'] = $db->getOne($sql_qq_qq87139667);
				}
			}
			if ($hahaha_zhyh)
			{
				$selected_first[$skey_ahxcjy] =  $hahaha_zhyh;
			}
			if ($sskey_www_ahxcjy_com!='-1')
			{
				$properties['spe'][$skey_ahxcjy]['values'][$sskey_www_ahxcjy_com]['selected_key_ahxcjy'] = "1";
			}
		}
		...
		$smarty->assign('pictures',            get_goods_gallery_attr_www_ahxcjy_com($goods_id, $goods_attr_id)); // 商品相册_修改 By www.ahxcjy.com
		$smarty->assign('new_goods',           get_recommend_goods('new'));     // 最新商品  改 By www.ahxcjy.com
 ...
 * @return  array
 */
function get_goods_gallery_attr_www_ahxcjy_com($goods_id, $goods_attr_id)
{
