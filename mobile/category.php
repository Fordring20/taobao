...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax') {
    ...
    if ($cat_id == 0)
    {
        $catidstr = get_catid_str();
        $children = "g.cat_id  IN " . $catidstr;
    }
    else
    {
...
//-- 心耀龙城
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'axqiuzhu')
{
...
//-- 心耀龙城事迹刷新
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajaxqiuzhu')
{
    include('includes/cls_json.php');
...
//-- 心耀龙城点赞
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'axqzxianax')
{
...
//-- 心耀龙城事迹详情
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'axqzinfo')
{
    setcookie('axqz_page', $page + 1);
...
if (!$smarty->is_cached('category.dwt', $cache_id)) {
    ...
    if ($cat_id == 0)
    {
        $catidstr = get_catid_str();
        $children = "g.cat_id  IN " . $catidstr;
        $ii = substr($catidstr, strpos($catidstr, "'") + 1, 1);
        $cat = get_cat_info($ii);
    }
    else
    {
...
$shankuan = get_order_shankuan();
$smarty->assign('shankuan', $shankuan);

$axymflag = empty($_REQUEST['aym']) ? 0 : intval($_REQUEST['aym']);
$smarty->assign('wap_group_ad',get_wap_group('手机端义卖广告1-1',1));  //手机端义卖广告1-1

setcookie('axqz_page', 1);

if ($axymflag == 1)
{
    $smarty->display('category_axym.dwt', $cache_id);
}
else
{
...
function get_order_shankuan()
{
    ...
    $sql = "SELECT order_id FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE add_time >= '" . $start . "' AND add_time <= '" . $end . "' and pay_status=2";
    $res = $GLOBALS['db']->getAll($sql);
    $orderids = array();
