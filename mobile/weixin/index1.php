...
if ($event['event'] == "subscribe") { //用户关注
	...
	if(intval($followInfo['ecuid']) == 0 && $weixinconfig['reg_type'] == 1){
		...
		$weixinconfig['followmsg'] .= "\r\n您已经注册过并绑定了帐号：$username 密码：$passwd_weixin\r\n请你及时修改完善您的用户资料，此账号密码可用于您在PC端登陆使用。";
	...
	switch($content){
		...
		case "info":
			...
			case "gn"://特色功能
				...
				$text .="\r\n<a href='{$weburl}mobile/index.php'>浏览手机商城</a>";
		...
		case 'qdcx':
			...
			break;
		case 'qrcode':
			$text = '正在为您生成专属推广二维码，请稍后...';
			echo $weixin->text($text)->reply();echo $weixin->text($text)->reply();
			$text = '处理完毕';
			echo $weixin->text($text)->reply();exit;
			break;
	}
