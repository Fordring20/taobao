...
function get_user_orders_ajax($user_id, $limit,$where='')
{
    ...
    foreach ($res as $key=>$row)
    {
        ...
        {  
            ...
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                ...
                // @$row['handler'] .= " <a href='kuaidi_list.php?order_id=$row[order_id]'>".$GLOBALS['_LANG']['view_wuliu'] ."</a>";
                @$row['handler'] .= " <a href='user.php?act=shouhou_order&order_id=$row[order_id]'>申请售后</a>";
	...
	$row['order_status1'] = $row['order_status'];
        ...
        if ($GLOBALS['_LANG']['os'][$row['order_status']] != '已确认')
        {
            $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']];
        }
        elseif ($GLOBALS['_LANG']['ps'][$row['pay_status']] != '已付款')
        {
		...
		$arr[$row['order_id']] = array('order_id'       => $row['order_id'],
						...
						'is_suborder' => $row['parent_order_id'] ? "(子订单)" : "",  //代码增加   By  bbs.ahxcjy.com
...
//				@$row_time = $okgoods_time - (local_date('d',gmtime()) - local_date('d',$row['shipping_time']));
            ...
            else if ($order['shipping_status'] == SS_RECEIVED)
            {
                ...
                // $handler['handler'] .= " <a href='kuaidi_list.php?order_id=$order[order_id]'>".$GLOBALS['_LANG']['view_wuliu'] ."</a>";
                $handler['handler'] .= " <a href='user.php?act=shouhou_order&order_id=$order[order_id]'>申请售后</a>";
