...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cartth') . " WHERE  $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND rec_id in (".$_SESSION['sel_cartgoods'].")";  //jx
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		foreach ($goods AS $val)
		{
           ...
           $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cartth') . " AS c left join ". $GLOBALS['ecs']->table('goods') ." AS g on c.goods_id=g.goods_id WHERE g.supplier_id = '". $row_supp['supplier_id'] ."' AND $sql_where AND c.extension_code != 'package_buy' AND c.is_shipping = 0 ".$sql_plus;  //jx
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        ...
        $sql = 'SELECT SUM(eg.exchange_integral) '.
               'FROM ' . $GLOBALS['ecs']->table('cartth') . ' AS c,' . $GLOBALS['ecs']->table('exchange_goods') . 'AS eg '.
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			...
			" WHERE $sql_where " .
            " AND c.rec_type = '$type' $id_ext ";  //代码修改 By  bbs.ahxcjy.com  增加一个 $id_ext , package_attr_id	
...
{
    /* 检查该商品是否已经存在在购物车中 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('cartth').
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cartth') . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "'";
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        ...
        /* 添加 */
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cartth'), $parent, 'INSERT');
    ...
    {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cartth').
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cartth') . " SET goods_number = '$num'" .
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cartth') . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
...
function clear_cart($type = CART_GENERAL_GOODS,$other='')
{
	...
	$sql = "DELETE FROM " . $GLOBALS['ecs']->table('cartth') .
...
function get_cart_goods($other='')
{
	...
	$sql = "SELECT c.*, g.cat_id, g.brand_id, c.extension_code, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid  " .
            " FROM " . $GLOBALS['ecs']->table('cartth') . " AS c left join " .$GLOBALS['ecs']->table('goods')." AS g ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id'] ); //修改 by www.ahxcjy.com 增加一个变量
        }
...
function check_consignee_info($consignee, $flow_type)
{
    if (exist_real_goods(0, $flow_type))
    {
        ...
        $res = !empty($consignee['consignee']) && !empty($consignee['mobile']);
...
function compute_discount($supplierid=-1)
{
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id, g.supplier_id " .
            "FROM " . $GLOBALS['ecs']->table('cartth') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " .
...
function get_cart_goods_info($rec_id){
    $sql = "select * from ". $GLOBALS['ecs']->table('cartth') ." where rec_id = $rec_id";
