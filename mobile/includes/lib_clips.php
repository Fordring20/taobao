...
function insert_user_account($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note,amount_txr_xm,amount_txr_yhkmc,amount_txr_yhkkh,amount_txr_zfb, process_type, payment, is_paid)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]','$surplus[amount_txr_xm]', '$surplus[amount_txr_yhkmc]','$surplus[amount_txr_yhkkh]','$surplus[amount_txr_zfb]','$surplus[process_type]', '$surplus[payment]', 0)";
    $GLOBALS['db']->query($sql);
...
function update_user_account($surplus)
{
    $sql = 'UPDATE ' .$GLOBALS['ecs']->table('user_account'). ' SET '.
		   ...
		   "amount_txr_xm  = '$surplus[amount_txr_xm]', ".
		    "amount_txr_yhkmc  = '$surplus[amount_txr_yhkmc]', ".
			 "amount_txr_yhkkh  = '$surplus[amount_txr_yhkkh]', ".
		    "amount_txr_zfb  = '$surplus[amount_txr_zfb]', ".
...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
			...
			$rows['amount_txr_xm']        = nl2br(htmlspecialchars($rows['amount_txr_xm']));
			$rows['amount_txr_yhkmc']        = nl2br(htmlspecialchars($rows['amount_txr_yhkmc']));
			$rows['amount_txr_yhkkh']        = nl2br(htmlspecialchars($rows['amount_txr_yhkkh']));
			$rows['amount_txr_zfb']        = nl2br(htmlspecialchars($rows['amount_txr_zfb']));
